    ...
    try {
      ...
      const response = await fetch('/api/llm', {
        ...
        body: JSON.stringify({
          message: chatInput,
          language: language,
          type: currentScreen === 'symptom-check' ? 'symptom-analysis' : 'general-health',
      ...
      const aiResponse = {
        role: 'ai',
        text: language === 'tamil'
          ? 'உங்கள் கேள்விக்கு நன்றி. தற்போது டெமோ பதில். விரைவில் முழு AI சேவை கிடைக்கும்.'
          : 'Thank you for your question. Demo response for now. Full AI service coming soon.',
        timestamp: new Date()
      };
      setChatMessages(prev => [...prev, aiResponse]);
    } finally {
